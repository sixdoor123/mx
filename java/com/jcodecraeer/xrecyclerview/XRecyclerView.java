...
public class XRecyclerView extends RecyclerView {
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled
                && !(mHeaderViews.get(0) instanceof ArrowRefreshHeader)) {
            ArrowRefreshHeader refreshHeader = new ArrowRefreshHeader(mContext);
            mHeaderViews.add(0, refreshHeader);
            mRefreshHeader = refreshHeader;
            mRefreshHeader.setProgressStyle(mRefreshProgressStyle);
        ...
        if (previousTotal < getLayoutManager().getItemCount()) {
            if (footView instanceof LoadingMoreFooter) {
                ((LoadingMoreFooter) footView)
                        .setState(LoadingMoreFooter.STATE_COMPLETE);
            } else {
            ...
            if (footView instanceof LoadingMoreFooter) {
                ((LoadingMoreFooter) footView)
                        .setState(LoadingMoreFooter.STATE_NOMORE);
            } else {
                footView.setVisibility(View.GONE);
            }
            isnomore = true;
...
//		 moreTextEnd.setText("�Ѿ�������");
    ...
    public void refresh() {
        mRefreshHeader.onMove(BaseActivity.getDensity_int(98));
// 		mRefreshHeader.setState(BaseRefreshHeader.STATE_RELEASE_TO_REFRESH);
        mRefreshHeader.setState(BaseRefreshHeader.STATE_REFRESHING);
    }

    public boolean isRefreshing() {
        return mRefreshHeader.getState() == BaseRefreshHeader.STATE_REFRESHING;
    }
...
//	}
    ...
    public void setLaodingMoreProgressStyle(int style) {
        ...
        if (mFootViews.size() > 0
                && mFootViews.get(0) instanceof LoadingMoreFooter) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            TLog.e("", layoutManager.getChildCount() + "---" + lastVisibleItemPosition + "---" + (layoutManager.getItemCount() - 1) + "---" + layoutManager.getItemCount() + "--" + layoutManager.getChildCount() + "---" + mRefreshHeader.getState());
    ...
    private class WrapAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public WrapAdapter(ArrayList<View> headerViews,
                           ArrayList<View> footViews, RecyclerView.Adapter adapter) {
        ...
        public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
            ...
            if (lp != null
                    ...
                    && (isHeader(holder.getLayoutPosition()) || isFooter(holder
                    .getLayoutPosition()))) {
        ...
        public boolean isFooter(int position) {
            return position < getItemCount()
                    && position >= getItemCount() - mFootViews.size();
        }
        ...
        public void removeHead(int postion) {
            if (mHeaderViews.size() > postion) {
                mHeaderViews.remove(postion);
                headerPosition = 1;
            }
        }
