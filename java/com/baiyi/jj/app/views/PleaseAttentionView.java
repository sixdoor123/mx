...
/**
 * �����������ע
 * ���ߣ�lizl on 2016/12/22 17:12
 */
public class PleaseAttentionView extends ViewGroup {
    ...
    public PleaseAttentionView(Context context) {
        this(context, null);
    }

    public PleaseAttentionView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PleaseAttentionView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setTextSize() {
        ...
        for (int i = 0; i < this.textViews.length; i++) {
            textViews[i].setTextSize(FontUtil.getLabelTextSize());
        }
    }
    public void setTags(List<String> tagList,float size) {
        this.tags = tagList;
        this.removeAllViews();
        textViews = new TextView[tagList.size()];
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                textViews[i] = (TextView) mInflater.inflate(mTagResId, null);
                if (mTagResId == DEFAULT_TAG_RESID) {
                    textViews[i].setBackgroundResource(mBackground);
                    textViews[i].setTextSize(size);
                    textViews[i].setTextColor(mTagColor);
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                textViews[i].setLayoutParams(layoutParams);
                textViews[i].setText(tags.get(i));
                textViews[i].setTag(TYPE_TEXT_NORMAL);
                textViews[i].setIncludeFontPadding(false);
                final int finalI = i;
