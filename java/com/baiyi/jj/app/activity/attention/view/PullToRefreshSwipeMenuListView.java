...
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mEnablePullLoad && mFooterView.getHeight()>0 && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
				...
				// invoke refresh
				TLog.e("list","eable=="+mEnablePullLoad+"=========="+mHeaderView.getVisiableHeight()+"---"+ mHeaderViewHeight);
				if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight && mFooterView.getmState() != PullToRefreshListFooter.STATE_LOADING) {
	...
	private void startLoadMore() {
		if (isNoMore){
			...
			if (mHeaderView.getmState() == PullToRefreshListHeader.STATE_REFRESHING){
				return;
			}
			mPullLoading = true;
	...
	public void refresh() {
		mHeaderView.onMove(BaseActivity.getDensity_int(80));
