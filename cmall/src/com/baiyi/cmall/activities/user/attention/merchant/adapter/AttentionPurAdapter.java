 ...
 * @param <T>
 */
public class AttentionPurAdapter<T> extends BaseRecycleViewAdapter<T> {

	public AttentionPurAdapter(Context context) {
		super(context);
	}
	...
	public void onInitViewHolder(
		...
		holder.category.setText(info.getGoodSCategory() + "/"
				+ info.getGoodSBrand());
		holder.quantity.setText(Utils.dealWeight(info.getGoodSWeight())
				+ "吨");
		String price = Utils.dealPrice(""
				+ (info.getGoodSPrePrice() == null ? 0 : info
						.getGoodSPrePrice()));
		if (price.startsWith(".")) {
			price = "0" + price;
		}
