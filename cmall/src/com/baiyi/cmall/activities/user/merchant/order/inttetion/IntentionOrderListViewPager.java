 ...
 * @author sunxy
 */
public class IntentionOrderListViewPager extends BasePageView implements OnClickListener, OnCheckedChangeListener {
	...
	public IntentionOrderListViewPager(Context context) {
		super(context);
	...
	public void initRg() {
		...
		View view = ContextUtil.getLayoutInflater(context).inflate(R.layout.activity_my_purchase_order, null);
		this.addView(view);
		// 找下面三角小图标
		mImgAllProviderChosed = (ImageView) view.findViewById(R.id.all_provider_choose);
		mImgNotAudirProvider = (ImageView) view.findViewById(R.id.not_audit_provider_choose);
		mImgAuditProviderChosed = (ImageView) view.findViewById(R.id.audit_provider_choose);
		mImgNotPassProviderChosed = (ImageView) view.findViewById(R.id.not_pass_provider_choose);

		myViewPager = (MyViewPager) findViewById(R.id.vp_pager);
		mRgMyOrderGroup = (RadioGroup) view.findViewById(R.id.rg_my_order);
		mRbAllOrderButton = (RadioButton) view.findViewById(R.id.rb_all_intent_order);
		mRbAllOrderButton.setChecked(true);
		mRbAllOrderButton.setText("全部");
		mRbNotSureOrderButton = (RadioButton) view.findViewById(R.id.rb_not_sure_intent_order);
		mRbNotSureOrderButton.setText("待付款");
		mRbSureOrderButton = (RadioButton) view.findViewById(R.id.rb_sure_intent_order);
		mRbSureOrderButton.setText("待收货");
		mRbCompleteOrderButton = (RadioButton) view.findViewById(R.id.rb_complete_intent_order);
		mRbCompleteOrderButton.setText("已完成");
		mRgMyOrderGroup.setOnCheckedChangeListener(this);

	}

	private IntentionOrderCategoryViewPager viewPager1;// 全部
	private IntentionOrderCategoryViewPager viewPager2;// 已确认
	private IntentionOrderCategoryViewPager viewPager3;// 未确认
	private IntentionOrderCategoryViewPager viewPager4;// 已完成
	...
	private void initViewPager() {
		...
		viewPager1 = new IntentionOrderCategoryViewPager(context, "0");
		viewPager2 = new IntentionOrderCategoryViewPager(context, "1");
		viewPager3 = new IntentionOrderCategoryViewPager(context, "2");
		viewPager4 = new IntentionOrderCategoryViewPager(context, "3");
