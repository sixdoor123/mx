 ...
 * 
 */
public class IntentionOrderView extends BasePageView implements
	...
	// 我的订单的适配器
	private IntentionOrderAdapter formAdapter;
	...
	public IntentionOrderView(Context context, int dealState) {
		super(context);
	...
	private void initContent() {
		...
		formAdapter = new IntentionOrderAdapter(context);
	...
	public void loadNetData() {
		...
		jsonLoader.setUrl(AppNetUrl.getPurIntentionFormListUrl(userId,
				dealState, pageIndex, Config.LIST_ITEM_COUNT));
		...
		jsonLoader.setLoaderListener(new LoaderListener() {
			...
			public void onCompelete(Object arg0, Object result) {
				...
				// 我发出的意向数据
				datas = MyPurFormManager.getMyPurchaseIntentionFormList(array);

				// 显示暂无数据
				if (Utils.isStringEmpty(datas) && pageIndex == 1) {
				...
				if (pageIndex == 1) {
					// 刷新第一页数据
					formAdapter.setDatas(datas);
					listView.refreshComplete();
				} else {
					// 添加下一页数据
					formAdapter.addDatas(datas);
					listView.loadMoreComplete();
				}
	...
	public void onItemClick(int position, View itemView) {
		...
		OrderEntity info = formAdapter.getDataList().get(position);

		/*
		 * 根据不同的订单状态跳转至不同的界面 状态：1,12-代付款 8--待收货 7--交易完成
		 */
		switch (info.getOrderState()) {
		case 1:
			((BaseActivity) context).goToActivity(
					ObligationIntentionOrderActivity.class,
		...
		default:
			((BaseActivity) context).goToActivity(
					OtherIntentionOrderActivity.class,
					info.getOrderStateName(), info.getId());
			break;
		}
